        ...
        <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-2xl p-6 space-y-6">
            ...
            <div className="flex bg-white rounded-xl p-1 shadow-sm">
                <button
                    ...
                    className={`flex-1 py-3 px-4 rounded-lg font-medium transition-all ${format === INVOICE_FORMATS.THREE_COPY
                            ? 'bg-purple-600 text-white shadow-md'
                            : 'text-gray-600 hover:bg-gray-100'
                        }`}
                >
                    <div className="text-lg">三聯式發票</div>
                    <div className={`text-xs ${format === INVOICE_FORMATS.THREE_COPY ? 'text-purple-200' : 'text-gray-400'}`}>
                        開給公司行號（需統編）
                    </div>
                </button>
                <button
                    onClick={() => setFormat(INVOICE_FORMATS.TWO_COPY)}
                    className={`flex-1 py-3 px-4 rounded-lg font-medium transition-all ${format === INVOICE_FORMATS.TWO_COPY
                            ? 'bg-amber-500 text-white shadow-md'
                            : 'text-gray-600 hover:bg-gray-100'
                        }`}
                >
                    <div className="text-lg">二聯式發票</div>
                    <div className={`text-xs ${format === INVOICE_FORMATS.TWO_COPY ? 'text-amber-200' : 'text-gray-400'}`}>
                        開給個人消費者
                    </div>
                </button>
            </div>
                    ...
                    <div className="flex gap-2">
                        {Object.entries(TAX_TYPES).map(([key, { label }]) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg font-medium transition-all ${taxType === key
            ...
            {calculations.total > 0 && (
                <div className="bg-white rounded-xl p-6 shadow-sm">
                    ...
                    <div className={`border-2 ${format === INVOICE_FORMATS.THREE_COPY ? 'border-purple-300' : 'border-amber-300'} rounded-lg p-6 bg-gradient-to-br ${format === INVOICE_FORMATS.THREE_COPY ? 'from-purple-50 to-white' : 'from-amber-50 to-white'}`}>
                        {/* 發票頭部 */}
                        <div className="text-center mb-4 pb-4 border-b-2 border-dashed border-gray-300">
                            <p className={`font-bold text-lg ${format === INVOICE_FORMATS.THREE_COPY ? 'text-purple-700' : 'text-amber-700'}`}>
                                {format === INVOICE_FORMATS.THREE_COPY ? '三聯式統一發票' : '二聯式統一發票'}
                            </p>
                            <p className="text-sm text-gray-500">（{format === INVOICE_FORMATS.THREE_COPY ? '收執聯' : '收執聯'}）</p>
                        </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center pt-2 border-t border-gray-200">
                                ...
                                <span className="font-bold text-xl text-green-600">${formatNumber(calculations.total)}</span>
                            </div>
            ...
            {calculations.total > 0 && (
                <div className="flex gap-2">
                    <button
                        onClick={() => copyToClipboard(`銷售額: ${formatNumber(calculations.priceWithoutTax)}, 稅額: ${formatNumber(calculations.taxAmount)}, 總計: ${formatNumber(calculations.total)}`, 'all')}
