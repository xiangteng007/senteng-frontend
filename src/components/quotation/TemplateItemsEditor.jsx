...
const SortableItemRow = ({ item, index, onUpdate, onRemove }) => {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-center gap-2 p-3 bg-white rounded-lg border transition-colors group ${
        isDragging ? 'border-orange-400 shadow-lg' : 'border-gray-100 hover:border-gray-200'
      }`}
    >
      {/* 拖曳手柄 */}
      <div
      ...
      {/* 序號 */}
      <div className="w-8 text-center text-xs text-gray-400 font-medium">{index + 1}</div>
...
const ChapterSection = ({
  ...
  const chapterTotal = chapterItems.reduce(
  ...
  return (
    <div className="bg-gray-50 rounded-xl p-4">
      ...
      >
        ...
        <div className="text-sm font-medium text-orange-600">${formatCurrency(chapterTotal)}</div>
      </div>
...
const TemplateItemsEditor = ({ items = [], onChange, chapters = [] }) => {
  ...
  const uniqueChapters =
    chapters.length > 0 ? chapters : [...new Set(items.map(item => item.chapter || '其他'))];
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between p-4 bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl border border-orange-100">
          ...
          <div className="text-right">
            ...
            <div className="text-xl font-bold text-orange-600">{formatCurrency(totalAmount)}</div>
          </div>
        </div>
      </div>
    );
