    ...
    >
      ...
      >
        <div className="flex items-center gap-2 sm:gap-3">
          <div
            className="hidden sm:block text-gray-300 cursor-grab active:cursor-grabbing p-1"
        ...
        <div className="text-right">
          <div
            className={`font-bold font-mono text-sm sm:text-base ${account.balance < 0 ? 'text-red-500' : 'text-gray-800'}`}
          >
        ...
        <div className="px-3 sm:px-4 pb-3 sm:pb-4 bg-gray-50/30 border-t border-gray-100 animate-slide-up">
          <div className="grid grid-cols-2 gap-3 sm:gap-4 text-sm mb-3 pt-3">
            ...
            <div>
              ...
              <div className="font-mono text-xs sm:text-sm text-gray-600 tracking-wide">
                {account.number}
              </div>
            </div>
          </div>
          <div className="flex justify-end gap-2">
            <button
              ...
              className="px-2 sm:px-3 py-1.5 text-xs bg-white border border-gray-200 rounded-lg hover:border-morandi-blue-300 text-gray-600 transition-colors"
            >
              編輯帳戶
            </button>
            <button
              ...
              className="px-2 sm:px-3 py-1.5 text-xs bg-morandi-text-accent text-white rounded-lg hover:bg-gray-800 transition-colors"
            >
              查看明細
            </button>
          </div>
      ...
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="flex bg-gray-100 rounded-lg p-0.5">
          {['週', '月', '年'].map(t => (
            <span
              key={t}
              className={`text-[10px] px-2 py-0.5 rounded cursor-pointer ${type === t ? 'bg-white shadow-sm font-bold' : 'text-gray-500'}`}
            >
              {t}
            </span>
          ))}
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-3">
        ...
        <button onClick={onAddTx} className="p-1 hover:bg-gray-100 rounded text-morandi-blue-600">
          <Plus size={16} />
        </button>
      </div>
      <div className="flex-1 overflow-y-auto space-y-2">
        {data.map(t => (
          ...
          >
            ...
            <div
              className={`font-mono font-bold ${t.type === '收入' ? 'text-morandi-green-600' : 'text-red-500'}`}
            >
              {t.type === '收入' ? '+' : '-'} {t.amount}
            </div>
          </div>
