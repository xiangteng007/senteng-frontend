    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
        {dailyEvents.length === 0 ? (
          <div className="text-gray-400 text-sm text-center py-4">無行程</div>
        ) : (
          dailyEvents.map(evt => (
            <div
              key={evt.id}
              className="flex gap-3 items-start p-2 rounded-lg hover:bg-gray-50 transition-colors border-l-2 border-morandi-blue-500 pl-2"
            >
              <div className="text-xs font-mono font-bold text-morandi-text-accent mt-0.5">
                {evt.time}
              </div>
    ...
    <div className="h-full flex flex-col bg-yellow-50/30 -m-4 p-4">
      <div className="flex-1 overflow-y-auto space-y-2 mb-2 custom-scrollbar">
        {memos.map((m, i) => (
          <div
            key={i}
            className="bg-white/80 p-2 shadow-sm rounded border border-yellow-100/50 flex justify-between group"
          >
            <span className="text-sm text-gray-700">{m}</span>
            <button
              onClick={() => setMemos(memos.filter((_, idx) => idx !== i))}
...
export const WidgetOverviewStats = ({ finance, projects, clients, size }) => {
  ...
  const monthlyIncome =
    finance?.transactions?.filter(t => t.type === '收入').reduce((acc, c) => acc + c.amount, 0) ||
    0;

  // Projects Calculation
  const activeProjects =
    projects?.filter(p => p.status === '施工中' || p.status === '進行中').length || 0;
      ...
      <div className="h-full flex flex-col justify-between">
        ...
        <div>
          <div className="text-2xl font-bold text-gray-800">
            ${(monthlyIncome / 10000).toFixed(1)}萬
          </div>
      ...
      <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-gray-800">
              {activeProjects} <span className="text-xs font-normal text-gray-400">案</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-gray-800">
              ${(totalBalance / 10000).toFixed(1)}萬
            </div>
          </div>
        </div>
      </div>
        ...
        {activities.length === 0 ? (
          ...
          activities.map(log => (
            <div key={log.id} className="flex gap-3 text-sm">
              <div className="w-12 text-[10px] text-gray-400 text-right pt-0.5 shrink-0">
                {log.time}
              </div>
              <div className="relative pl-3 border-l border-gray-100">
                <div
                  className={`absolute left-[-2.5px] top-1.5 w-1.5 h-1.5 rounded-full ${log.type === 'finance' ? 'bg-green-400' : 'bg-morandi-blue-400'}`}
                ></div>
