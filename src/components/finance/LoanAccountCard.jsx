...
export const LoanAccountCard = ({ loan, onEdit, onViewDetails, onRecordPayment }) => {
  ...
  return (
    <div
      className={`bg-white rounded-2xl border ${borderColor} shadow-sm transition-all duration-300 mb-3 overflow-hidden`}
    >
      {/* 主要資訊區 */}
      <div
      ...
      <div className="px-4 pb-2">
        ...
        {loan.status === 'active' && daysUntilPayment <= 7 && (
          <div
            className={`flex items-center gap-2 text-xs mt-2 p-2 rounded-lg ${daysUntilPayment <= 3 ? 'bg-orange-50 text-orange-700' : 'bg-blue-50 text-blue-700'}`}
          >
            <AlertCircle size={14} />
            <span>
        ...
        <div className="px-4 pb-4 bg-gray-50/50 border-t border-gray-100 animate-slide-up">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm mb-3">
            ...
            <div>
              ...
              <span
                className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[loan.status] || statusColors.active}`}
              >
                {loan.status === 'active'
                  ? '還款中'
