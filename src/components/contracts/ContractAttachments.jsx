    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-5xl h-[90vh] flex flex-col">
        ...
        <div className="flex-1 p-4 bg-gray-100">
          <iframe src={pdfUrl} className="w-full h-full rounded-lg border-0" title={fileName} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg group hover:bg-gray-100 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium text-sm text-gray-800 truncate">{attachment.fileName}</p>
        <p className="text-xs text-gray-400">
          {formatFileSize(attachment.size)} ·{' '}
          {new Date(attachment.uploadedAt).toLocaleDateString('zh-TW')}
        </p>
      </div>
...
const ContractAttachments = ({ contractId, addToast }) => {
  ...
  const saveAttachments = newAttachments => {
    ...
    localStorage.setItem(`contract_attachments_${contractId}`, JSON.stringify(newAttachments));
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-400">{attachments.length} 個檔案</span>
      </div>

      {/* 上傳區域 */}
      <UploadDropzone onFilesSelected={handleUpload} uploading={uploading} disabled={uploading} />
