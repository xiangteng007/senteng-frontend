  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        <span className="font-medium text-sm">{isOnline ? '連線中' : '離線模式'}</span>
        {queueStats.pending > 0 && (
          <span className="text-xs opacity-80">{queueStats.pending} 筆待同步</span>
        )}
        {queueStats.failed > 0 && (
        ...
        {offlineDuration && offlineDuration > 0 && (
          <span className="text-xs opacity-60">離線 {offlineDuration} 秒</span>
        )}
      </div>
