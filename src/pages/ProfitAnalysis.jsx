...
const BigStatCard = ({ icon: Icon, label, value, subValue, trend, color = 'gray' }) => (
  <div
    className={`bg-gradient-to-br from-${color}-500 to-${color}-600 rounded-2xl p-6 text-white shadow-lg`}
  >
  ...
  return (
    <div className="space-y-2">
      {alerts.map((alert, i) => (
        <div
          key={i}
          className={`p-3 rounded-lg border ${getAlertBg(alert.type)} flex items-start gap-3`}
        >
        ...
        <div key={project.contractId} className="p-4 bg-gray-50 rounded-xl">
          <div className="flex items-center justify-between mb-2">
            ...
            <span
              className={`text-lg font-bold ${
                parseFloat(project.marginRate) < 10
                  ? 'text-red-600'
                  : parseFloat(project.marginRate) < 20
                    ? 'text-yellow-600'
                    : 'text-green-600'
              }`}
            >
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            ...
            <div>
              ...
              <p
                className={`font-medium ${project.grossProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}
              >
...
const StatusDistribution = ({ distribution }) => {
  ...
  if (total === 0) {
    return <div className="text-center py-4 text-gray-400 text-sm">尚無專案資料</div>;
  }
...
const ProfitAnalysis = ({ addToast }) => {
  ...
  return (
    <div className="space-y-6">
      {/* 標題 */}
      <SectionTitle icon={BarChart3} title="利潤分析" subtitle="專案成本、收入與毛利綜合分析" />
