...
// 提取的 Widget 組件
import { StatCard, ClientRow, ContactLogItem, STATUS_CONFIG } from '../components/widgets/ClientWidgets';
...
const Clients = ({ data = [], loading, addToast, onUpdateClients, allProjects = [] }) => {
  ...
  const handleAddContactLog = async () => {
    ...
    try {
      const updatedLogs = [
        ...(activeClient.contactLogs || []),
        { ...newContactLog, id: Date.now() },
      ];
                    ...
                    <>
                      {activeClient.phone && (
                        <span className="flex items-center gap-1">
                          <Phone size={14} /> {activeClient.phone}
                        </span>
                      )}
                      {activeClient.email && (
                        <span className="flex items-center gap-1">
                          <Mail size={14} /> {activeClient.email}
                        </span>
                      )}
                    </>
                  )}
            ...
            {isEditing ? (
              <div className="space-y-4">
                ...
                <InputField
                  ...
                  placeholder="例：朋友介紹、網路搜尋"
                />
                <DynamicFieldEditor
                  fields={editFormData.customFields || []}
                  onChange={newFields =>
                    setEditFormData({ ...editFormData, customFields: newFields })
                  }
                />
              </div>
            ) : (
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {(activeClient.contactLogs || []).length > 0 ? (
                activeClient.contactLogs
                  .slice()
                  .reverse()
                  .map((log, idx) => <ContactLogItem key={idx} log={log} />)
              ) : (
                <div className="text-sm text-gray-400 text-center py-8">尚無聯絡記錄</div>
              )}
            </div>
          </Card>
          ...
          <Card className="lg:col-span-3">
            ...
            {(() => {
              const relatedProjects = allProjects.filter(
                p => p.client === activeClient.name || p.clientId === activeClient.id
              );
              return relatedProjects.length > 0 ? (
                ...
                <div className="text-sm text-gray-400 text-center py-8">尚無關聯專案</div>
              );
            })()}
          </Card>
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <InputField
            ...
            onChange={e => setNewClientData({ ...newClientData, budget: e.target.value })}
            placeholder="例：200-300萬"
          />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <InputField
            label="LINE ID"
            value={newClientData.lineId}
            onChange={e => setNewClientData({ ...newClientData, lineId: e.target.value })}
            placeholder="例：@lineid 或 名稱"
          />
          <div>
        ...
        <div className="border-t border-gray-100 pt-4 mt-4">
          ...
          <DynamicFieldEditor
            fields={newClientData.customFields}
            onChange={newFields => setNewClientData({ ...newClientData, customFields: newFields })}
          />
        </div>
