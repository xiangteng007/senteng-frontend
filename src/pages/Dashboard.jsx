...
const formatDate = () => {
    ...
    return { full: `${year}年 ${month}月 ${day.toString().padStart(2, '0')}日`, weekday: `星期${weekdays[d.getDay()]}` };
};
...
const Dashboard = ({ events = [], finance = {}, projects = [], clients = [] }) => {
    ...
    // Calculate stats
    const activeProjectsCount = projects.filter(p => ['設計中', '施工中', '進行中'].includes(p.status)).length;
    ...
    const monthlyIncome = finance.transactions
        ?.filter(t => t.type === '收入')
                            ...
                            {projects.slice(0, 2).map(project => (
                                <Card key={project.id} className="hover:shadow-md transition-shadow cursor-pointer">
                                    <div className="flex justify-between items-start mb-3">
                                        ...
                                        <Badge color={project.status === '施工中' ? 'gray' : 'gray'}>{project.status}</Badge>
                                    </div>
                ...
                <div>
                    ...
                    <Card className="h-full max-h-[400px] overflow-y-auto">
                        <ul className="space-y-4">
                            {memos.map(memo => (
                                <li key={memo.id} className="flex items-start gap-3 group cursor-pointer" onClick={() => toggleMemo(memo.id)}>
                                    {memo.done ? (
                                    ...
                                    <span className={`text-sm leading-relaxed ${memo.done ? 'text-gray-400 line-through' : 'text-gray-700'}`}>
                                        {memo.text}
                                    </span>
                                </li>
                            ))}
