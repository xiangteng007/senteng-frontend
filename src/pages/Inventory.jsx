...
const ItemModal = ({ isOpen, onClose, item, onSave, isEdit }) => {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      safeStock: parseInt(form.safeStock),
      status,
    });
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        <StatCard icon={Box} label="總品項數" value={stats.total} color="blue" />
        <StatCard
          icon={AlertTriangle}
          label="庫存偏低"
        ...
        />
        <StatCard icon={TrendingDown} label="本月出庫" value={stats.monthOut} color="purple" />
      </div>
                    ...
                    <td className="p-4">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-xs text-gray-400">
                          {item.mainCategory || getMainCategory(item.category)}
                        </span>
            ...
            {movements
              ...
              .map(m => (
                ...
                >
                  ...
                  <div className="text-right">
                    <div
                      className={`font-bold ${m.type === '入' ? 'text-green-600' : 'text-purple-600'}`}
                    >
                      {m.type === '入' ? '+' : '-'}
                      {m.quantity}
                    </div>
