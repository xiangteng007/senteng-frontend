...
export const CostEstimator = ({
  ...
  const updateQuantity = (id, quantity) => {
    if (quantity <= 0) {
      ...
      setEstimateItems(items => items.map(item => (item.id === id ? { ...item, quantity } : item)));
    }
  };
  ...
  const exportToSheet = async () => {
    ...
    const name =
      estimateName.trim() ||
      `估算清單_${new Date().toLocaleDateString('zh-TW').replace(/\//g, '-')}`;
        ...
        <div className="bg-gray-50 rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-gray-400">
              {selectedCategory} / {currentMaterials.length}項
            </span>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <input
                      ...
                      onChange={e => {
                        const newPrice =
                          parseFloat(e.target.value) || material.suggestedPrice || material.price;
          ...
          {hoveredMaterial ? (
            <div className="text-sm">
              <div className="font-medium text-gray-800 mb-1">
                {hoveredMaterial.name}{' '}
                <span className="text-xs text-gray-500">({hoveredMaterial.spec})</span>
              </div>
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 flex gap-3">
        ...
        <div className="text-sm text-orange-800">
          ...
          <p className="text-orange-600">
            選擇物料類別，點擊加入估算清單，系統將自動計算總價。可連結 Google Drive 同步物料資料庫。
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            ...
            <div className="text-xs text-gray-400 text-right">共 {estimateItems.length} 項材料</div>
