...
const StatusBadge = ({ status }) => (
  <span
    className={`px-2 py-1 rounded-full text-xs font-medium ${CHANGE_ORDER_STATUS_COLORS[status]}`}
  >
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold">{changeOrder?.changeOrderNo || '新增變更單'}</h2>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="p-4 bg-red-50 rounded-lg text-center">
            ...
            <p className="text-xl font-bold text-red-600">
              -{formatCurrency(totals.totalDeducted)}
            </p>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-2xl font-bold text-green-600">
            +{formatCurrency(cumulative.totalAdded)}
          </p>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 text-center">
          <p className="text-sm text-red-600">累計減項</p>
          <p className="text-2xl font-bold text-red-600">
            -{formatCurrency(cumulative.totalDeducted)}
          </p>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 text-center">
          <p className="text-sm text-blue-600">淨變更</p>
          <p
            className={`text-2xl font-bold ${cumulative.netChange >= 0 ? 'text-green-600' : 'text-red-600'}`}
          >
            {cumulative.netChange >= 0 ? '+' : ''}
            {formatCurrency(cumulative.netChange)}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            {changeOrders.map(order => {
              ...
              return (
                <tr key={order.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td
                    className={`py-3 px-4 text-right font-medium ${order.netChange >= 0 ? 'text-green-600' : 'text-red-600'}`}
                  >
                    {order.netChange >= 0 ? '+' : ''}
                    {formatCurrency(order.netChange)}
                  </td>
        ...
        {changeOrders.length === 0 && (
          <div className="text-center py-8 text-gray-400">尚無變更單，請點擊「新增變更單」建立</div>
        )}
      </div>
    </div>
  );
