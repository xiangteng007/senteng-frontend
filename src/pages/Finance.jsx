...
const Finance = ({
  ...
  const handleDragStartAccount = (e, idx) => {
    dragItem.current = idx;
  };
  ...
  const openAddAcc = () => {
    setEditingAcc(null);
    setNewAcc({ name: '', bank: '', number: '', balance: 0 });
    ...
    switch (w.type) {
        ...
        return (
          <div className="h-full flex flex-col">
            <div className="flex justify-end mb-2">
              <button
                onClick={openAddAcc}
                className="text-xs text-morandi-blue-600 hover:underline flex items-center gap-1"
              >
                <Plus size={12} /> 新增帳戶
              </button>
            </div>
      ...
      case 'finance-loans':
        ...
        const totalLoanAmount = loans.reduce(
          (sum, l) => sum + (l.remainingPrincipal || l.principalAmount || 0),
          0
        );
        return (
          <div className="h-full flex flex-col">
      ...
      case 'finance-trend':
        return <WidgetFinanceTrend size={w.size} />;
      case 'finance-tx':
        return (
          <WidgetFinanceTransactions
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div
        className={`grid grid-cols-1 lg:grid-cols-2 gap-6 auto-rows-auto ${isEditMode ? 'ring-2 ring-orange-300 ring-offset-4 rounded-2xl p-2 bg-orange-50/50' : ''}`}
      >
        {widgets.map((w, i) => (
          <WidgetWrapper
            key={w.id}
      ...
      >
        <div className="flex gap-4 mb-6 bg-gray-50 p-1 rounded-xl">
          <button
            onClick={() => setNewTx({ ...newTx, type: '收入', category: '' })}
          ...
          <button
            onClick={() => setNewTx({ ...newTx, type: '支出', category: '' })}
