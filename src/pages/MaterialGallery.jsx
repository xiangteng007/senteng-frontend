...
export const MaterialGallery = ({ addToast }) => {
  ...
  const handleAddCategory = async () => {
    ...
    if (galleryRoot) {
      const categoryFolder = await GoogleService.createDriveFolder(
        newCategory,
        galleryRoot.folderId
      );
    ...
    addToast?.(
      `類別「${newCategory}」已建立${driveFolderUrl ? '，Drive資料夾已同步' : ''}`,
      'success'
    );
  };
  ...
  const handleAddMaterial = async () => {
    ...
    setCategories(
      categories.map(cat =>
        cat.id === selectedCategory ? { ...cat, materials: [...cat.materials, material] } : cat
      )
    );
            ...
            {category.materials.map(m => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-2 sm:p-3">
                  ...
                  {m.description && (
                    <div className="text-white/80 text-[10px] truncate">{m.description}</div>
                  )}
                  ...
                  <div className="flex gap-1 mt-1">
                    <button
                      onClick={e => {
                        e.stopPropagation();
                        openEditMaterial(m, category.id);
                      }}
                    ...
                    <button
                      onClick={e => {
                        e.stopPropagation();
                        openDeleteConfirm('material', category.id, m.id);
                      }}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <div className="font-medium text-sm">
                  {selectedMaterial.type === 'image' ? '圖片' : '連結'}
                </div>
              </div>
            </div>
              ...
              {selectedMaterial.url && (
                ...
                >
                  ...
                  <span className="truncate flex-1">
                    {selectedMaterial.type === 'image' ? '開啟原圖' : '開啟連結'}
                  </span>
        ...
        {editingMaterial && (
          <div className="space-y-4">
            ...
            <InputField
              ...
              onChange={e =>
                setEditingMaterial({ ...editingMaterial, externalLink: e.target.value })
              }
              placeholder="選填，如廠商官網、購買連結等"
            />
          </div>
