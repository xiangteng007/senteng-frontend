...
const BimManagement = () => {
    ...
    const filteredModels = MOCK_BIM_MODELS.filter(model => {
        ...
        const matchesDiscipline = selectedDiscipline === 'all' || model.discipline === selectedDiscipline;
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex gap-2 border-b border-gray-100">
                ...
                <button
                    ...
                    className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'issues'
                        ? 'border-gray-800 text-gray-800'
                        : 'border-transparent text-gray-500 hover:text-gray-700'
                        }`}
                >
                ...
                <button
                    ...
                    className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'quantities'
                        ? 'border-gray-800 text-gray-800'
                        : 'border-transparent text-gray-500 hover:text-gray-700'
                        }`}
                >
                        ...
                        <Card key={model.id} className="hover:shadow-md transition-shadow cursor-pointer">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <Badge color={STATUS_COLORS[model.status]}>
                                    {model.status === 'ACTIVE' ? '使用中' : model.status === 'IN_REVIEW' ? '審核中' : '已封存'}
                                </Badge>
                            </div>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredIssues.map(issue => (
                                <tr key={issue.id} className="hover:bg-gray-50/50">
                                    ...
                                    <td className="px-6 py-4">
                                        <Badge color={PRIORITY_COLORS[issue.priority]}>
                                            {issue.priority}
                                        </Badge>
                                    </td>
